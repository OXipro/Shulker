...
public abstract class ShulkerClusterAPI {
    ...
    /**
     * Lists the unique IDs of players currently connected to the
     * provided server. Useful to build cross-server features such as
     * "swap to another lobby" without needing a Bukkit-side player list.
     *
     * @param serverName Server identifier
     * @return Unique IDs of players currently on that server
     */
    abstract public @NotNull List<@NotNull UUID> getPlayersInServer(@NotNull String serverName);
